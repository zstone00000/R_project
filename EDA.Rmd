...
```{r}
source('rat_data.R')
rat_data = import_ratdata('data/rat_data2012-2014.json')
rat = rat_data
```

# EDA

## XXX Add EDA from preventable hospitalizations to indicate importance
...
```{r}
spplot(pm.1214.winter.avg.spdf, zcol = 'avg_mcg_per_cm3')
```
...
ggplot() +
  ...
  labs(title = "Industrial zones vs. Asthma rate")
```
...
## XXX Do survey data here for correlation between vermin/sanitation issues and asthma
